...
export function BrushPlot({
  ...
  provenanceState,
  updateState = () => null,
}: StimulusParams<BrushParams, { all: { brush: BrushState } }> & {
  updateState: (b: BrushState) => void;
}) {
  const [filteredTable, setFilteredTable] = useState<ColumnTable | null>(null);
  const [brushState, setBrushState] = useState<BrushState>(
    provenanceState
      ? provenanceState.all.brush || provenanceState.all
      : {
          hasBrush: false,
          x1: 0,
          y1: 0,
  ...
  const { actions, trrack } = useMemo(() => {
    ...
    const brushMove = reg.register(
      "brushMove",
      (state, currBrush: BrushState) => {
        state.all = { brush: currBrush };
        return state;
      },
    );

    const brushResize = reg.register(
      "brushResize",
      (state, currBrush: BrushState) => {
        state.all = { brush: currBrush };
        return state;
  ...
  const moveBrushCallback = useCallback(
    (selType: SelectionType, state: BrushState) => {
      if (selType === "drag") {
  ...
  const debouncedCallback = useMemo(
    () => debounce(moveBrushCallback, 100, { maxWait: 100 }),
    [moveBrushCallback],
  );

  // brush callback, updating state, finding the selected points, and pushing to trrack
  const brushedSpaceCallback = useCallback(
    (
      sel: [[number | null, number | null], [number | null, number | null]],
      xScale: any,
      yScale: any,
      ...
      if (selType === "clear") {
        ...
        _filteredTable = fullTable!.filter(
          escape((d: any) => idSet.has(d[parameters.ids])),
        );
      } else if (parameters.brushType === "Axis Selection") {
        _filteredTable = fullTable!.filter(
          escape(
            (d: any) =>
              new Date(d[parameters.x]) >= new Date(xMin) &&
              new Date(d[parameters.x]) <= new Date(xMax) &&
              d[parameters.y] >= yMin &&
              d[parameters.y] <= yMax,
          ),
        );
      ...
      const newState = {
        ...
        ids:
          selType !== "clear" ? (_filteredTable?.array("id") as string[]) : [],
      };
