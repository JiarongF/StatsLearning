...
export function Paintbrush({
      ...
      svg.on("mousemove", (e: React.MouseEvent) => {
        ...
        if (isBrushing) {
          const selected = data.filter(
            (car) =>
              Math.abs(xScale(car[params.x]) - pos[0]) < BRUSH_SIZE &&
              Math.abs(yScale(car[params.y]) - pos[1]) < BRUSH_SIZE,
          );
          ...
          if (e.ctrlKey || e.metaKey || !isSelect) {
            ...
            const newIds = Array.from(
              new Set([
                ...brushState.ids,
                ...selected.map((car) => car[params.ids]),
              ]),
            );
  ...
  return (
    <circle
      style={{ cursor: isBrushing ? "pointer" : "default" }}
      r={BRUSH_SIZE}
      fill="darkgray"
      opacity={0.5}
      cx={brushPosition[0]}
