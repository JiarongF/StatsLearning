...
export function Scatter({
  ...
  setBrushedSpace: (
    brush: [[number | null, number | null], [number | null, number | null]],
    xScale: any,
    yScale: any,
    selType: "drag" | "handle" | "clear" | null,
    ids?: string[],
  ) => void;
  ...
  const xScale = useMemo(() => {
    ...
    if (params.dataType === "date") {
      return d3
        .scaleTime([margin.left, width + margin.left])
        .domain([new Date("2014-12-20"), new Date("2016-01-10")]);
    }

    return d3
      .scaleLinear([margin.left, width + margin.left])
      .domain([xMin - range / 10, xMax + range / 10])
      .nice();
  ...
  const yScale = useMemo(() => {
    ...
    return d3
      .scaleLinear([height + margin.top, margin.top])
      .domain([yMin - range / 10, yMax + range / 10])
      .nice();
  }, [height, yMax, yMin]);
  ...
  const clearCallback = useMemo(() => {
    ...
    if (brushType === "Axis Selection") {
      const brushX = d3
        .brushX()
        .extent([
          [margin.left, margin.top + height - 5],
          [margin.left + width, margin.top + height + 5],
        ])
        .on("brush end", (e) => {
      ...
      const brushY = d3
        .brushY()
        .extent([
          [margin.left - 5, margin.top],
          [margin.left + 5, margin.top + height],
        ])
        .on("brush end", (e) => {
          if (e.sourceEvent !== undefined) {
            setBrushedSpace(
      ...
      if (brushXRef.current && brushYRef.current) {
        ...
        if (!brushState.hasBrush) {
          d3.select(brushYRef.current).call(brushY.move, [
            yScale(yMax),
            yScale(yMin),
          ]);
          d3.select(brushXRef.current).call(brushX.move, [
            xScale(new Date("2015-01-02")),
            xScale(new Date("2015-12-31")),
          ]);
          setBrushedSpace(
            [
              [xScale(new Date("2015-01-02")), yScale(yMax)],
              [xScale(new Date("2015-12-31")), yScale(yMin)],
            ],
            xScale,
            yScale,
            "drag",
      ...
      const brush = d3
        ...
        .on("brush", (e) => {
          if (e.sourceEvent !== undefined) {
            setBrushedSpace(
              [
                [e.selection[0][0], e.selection[0][1]],
                [e.selection[1][0], e.selection[1][1]],
              ],
              xScale,
              yScale,
              e.mode,
        ...
        .on("end", (currData) => {
          if (
            currData.selection === null &&
            currData.sourceEvent !== undefined
          ) {
  ...
  const brushedSet = useMemo(
    () => (brushedPoints.length === 0 ? null : new Set(brushedPoints)),
    [brushedPoints],
  );
    ...
    return data.map((d, i) => {
      ...
      const xVal =
        params.dataType === "date"
          ? xScale(new Date(d[params.x]))
          : xScale(d[params.x]);

      return (
        <circle
          key={i}
          opacity={brushedSet && !brushedSet.has(d[params.ids]) ? 0.3 : 1}
          r={3}
          fill={
            brushedSet && !brushedSet.has(d[params.ids])
              ? "lightgray"
              : colorScale(d[params.category])
          }
          cx={xVal}
          cy={yScale(d[params.y])}
        />
      );
    });
  }, [
  ...
  return (
    <Stack>
      ...
      <Group style={{ width: "100%", height: "100%" }} wrap="nowrap">
        <svg
          id="scatterSvgBrushStudy"
          ref={ref}
          style={{
            height: "500px",
            width: params.brushType === "Axis Selection" ? "800px" : "530px",
            fontFamily:
        ...
        >
          {xScale && yScale ? (
            <g>
              ...
              <XAxisBar
                ...
                label={params.x}
                ticks={xScale
                  .ticks(params.brushType === "Axis Selection" ? 12 : 5)
                  .map((value) => ({
          ...
          {xScale && yScale && brushType === "Paintbrush Selection" ? (
            <Paintbrush
              brushState={brushState}
              setBrushedSpace={setBrushedSpace}
              params={params}
              data={data}
              isSelect={isPaintbrushSelect}
            ...
            <Stack gap={0}>
              ...
              <RangeSlider
                ...
                onChange={(value) => {
                  setBrushedSpace(
                    [
                      [xScale(value[0]), brushState.y1],
                      [xScale(value[1]), brushState.y2],
                    ],
                    xScale,
                    yScale,
                    "drag",
                ...
                value={
                  [
                    xScale.invert(brushState.x1),
                    xScale.invert(brushState.x2),
                  ] as any
                }
              />
            </Stack>
            <Stack gap={0}>
              ...
              <RangeSlider
                ...
                onChange={(value) => {
                  setBrushedSpace(
                    [
                      [brushState.x1, yScale(value[1])],
                      [brushState.x2, yScale(value[0])],
                    ],
                    xScale,
                    yScale,
                    "drag",
                ...
                style={{ width: "300px" }}
                marks={yScale.ticks(5).map((t) => ({
                  value: t,
                  label: t,
                }))}
                value={[
                  yScale.invert(brushState.y2),
                  yScale.invert(brushState.y1),
                ]}
              />
            </Stack>
          </Stack>
