...
export default function dynamic({
  ...
  const filteredAnswers = Object.entries(answers).filter(
    ([key, value]) =>
      key.startsWith(`${currentBlock}_${currentStep}`) && value.endTime > -1,
  );
  ...
  const checkCorrectness = (answer: StoredAnswer) =>
    (answer.answer.buttonResponse === "Left" &&
      answer.parameters.left > answer.parameters.right) ||
    (answer.answer.buttonResponse === "Right" &&
      answer.parameters.right > answer.parameters.left) ||
    (answer.answer.buttonResponse === "Same" &&
      answer.parameters.left === answer.parameters.right);
  ...
  if (lastAnswer) {
    ...
    if (isCorrect) {
      ...
      if (
        lastAnswer.answer.buttonResponse === "Same" &&
        lastAnswer.parameters.left === lastAnswer.parameters.right
      ) {
        return { component: null };
      }
    } else {
  ...
  const leftValue = lastAnswer
    ? isCorrect
      ? Math.min(100, lastLeftValue + 10)
      : Math.max(0, lastLeftValue - 10)
    : lastLeftValue;
  ...
  const rightValue = lastAnswer
    ? isCorrect
      ? Math.max(0, lastRightValue - 10)
      : Math.min(100, lastRightValue + 10)
    : lastRightValue;
