  ...
  return (
    <Card padding="md" radius="md">
      <Stack gap="sm">
        ...
        <Text size="sm"><strong>•</strong> Use the slider to adjust the correlation strength for generated (green) points.</Text>
        <Text size="sm"> <strong>•</strong> Click anywhere on the chart to add your own points (blue) at any time.</Text>
        <Text size="sm"> <strong>•</strong> Watch how the overall correlation coefficient changes as you experiment.</Text>
...
<Collapse in={showRInfo}>
  <Card shadow="xs" padding="sm" radius="sm" withBorder mt={6}>
    <Text size="sm">
      <strong>r</strong> is the correlation coefficient. In this tool it always ranges from 
      <strong> 0</strong> (no relationship) to <strong>1</strong> (perfect positive relationship).
    </Text>
    <Text size="sm" c="dimmed" mt={4}>
      Later you may encounter negative <strong>r</strong> values, which describe downward trends — 
      but here we’ll focus on positive ones.
        ...
        <Group justify="space-between" align="flex-start" mt="sm">
          <Stack gap="xs">
            <Text fw={500}>Total points: {generatedPoints.length + userPoints.length}</Text>
            {generatedR !== null && (
              <Text size="sm" c="green">Generated points r = {generatedR.toFixed(2)}</Text>
            )}
          </Stack>
          ...
          <Group gap="xs" align="center">
            {smoothR !== null && (
              ...
              >
                Overall r = {smoothR.toFixed(2)} → <em>{interpretCorrelation(smoothR)}</em>
              </Text>
            )}
            {userPoints.length > 0 && (
        ...
        <Text size="sm" c="dimmed">
          <strong>Play around: </strong> Set the slider to 1, then place points wherever you like. See how the correlation changes when you go against the trend.
        </Text>
      </Stack>
    </Card>
  );
