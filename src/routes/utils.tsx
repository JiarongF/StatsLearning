...
const modules = import.meta.glob("../public/**/*.{mjs,js,mts,ts,jsx,tsx}", {
  eager: true,
});
...
export function useCurrentComponent(): string {
  ...
  const [indexWhenSettingComponentName, setIndexWhenSettingComponentName] =
    useState<number | null>(null);

  const currentComponent = useMemo(
    () =>
      typeof currentStep === "number"
        ? getComponent(flatSequence[currentStep], studyConfig)
        : currentStep.includes("reviewer-") || currentStep.startsWith("__")
          ? currentStep
          : null,
    [currentStep, flatSequence, studyConfig],
  );

  const [compName, setCompName] = useState("__dynamicLoading");

  const nextFunc:
    | ((params: JumpFunctionParameters<unknown>) => JumpFunctionReturnVal)
    | null = useMemo(() => {
    if (typeof currentStep === "number" && !currentComponent) {
      const block = findFuncBlock(
        flatSequence[currentStep],
      ...
      const newFunc =
        reactPath in modules
          ? (modules[reactPath] as ModuleNamespace).default
          : null;
  ...
  useEffect(() => {
    if (!funcIndex && nextFunc && typeof currentStep === "number") {
      navigate(
        `/${studyId}/${encryptIndex(currentStep)}/${encryptIndex(0)}${window.location.search}`,
      );
    }
  ...
  useEffect(() => {
    if (typeof currentStep === "number") {
      ...
      const currentAnswer = Object.entries(_answers).find(
        ([key, _]) =>
          key.startsWith(`${funcName}_${currentStep}_`) &&
          key.endsWith(`${decryptedFuncIndex}`),
      );
      const answerCompName = currentAnswer
        ? currentAnswer[1].componentName
        : null;
      ...
      if (!component && nextFunc !== null) {
        const {
          ...
          answers: _answers,
          customParameters: findFuncBlock(
            flatSequence[currentStep],
            studyConfig.sequence,
          )?.parameters,
          currentStep,
        ...
        if (currCompName !== null) {
          ...
          navigate(
            `/${studyId}/${encryptIndex(currentStep + 1)}${window.location.search}`,
          );
        }
      }
    }
  ...
  if (currentComponent) {
    return typeof currentStep === "number"
      ? flatSequence[currentStep]
      : currentStep.replace("reviewer-", "");
  }

  return indexWhenSettingComponentName !== null &&
    funcIndex &&
    decryptIndex(funcIndex) === indexWhenSettingComponentName
    ? compName
    : "__dynamicLoading";
}
...
export function useCurrentIdentifier(): string {
  ...
  const identifier = useMemo(
    ...
    [currentComponent, currentStep, funcIndex, participantSequence],
  );
