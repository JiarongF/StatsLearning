...
function checkAllAnswersCorrect(
  ...
  const foundConfigComponent = Object.entries(studyConfig.components).find(
    ([configComponentId]) => configComponentId === componentName,
  );
  const foundConfigComponentConfig = foundConfigComponent
    ? foundConfigComponent[1]
    : null;

  if (!foundConfigComponentConfig) {
    throw new Error(
      `Component ${componentName} could not be found in the study components.`,
    );
  }
  ...
  return foundConfigComponentConfig.correctAnswer.every(
    (correctAnswerEntry) =>
      answers[correctAnswerEntry.id] === correctAnswerEntry.answer,
  );
}
...
export function useNextStep() {
  ...
  const dataCollectionEnabled = useMemo(
  ...
  const isNextDisabled =
    typeof currentStep !== "number" || isAnalysis || !areResponsesValid;
  ...
  const goToNextStep = useCallback(
    (collectData = true) => {
      ...
      const currentWindowEvents =
        windowEvents && "current" in windowEvents && windowEvents.current
      ...
      const hasSkipBlock =
        ...
        blocksForStep.some(
          (block) =>
            Object.hasOwn(block.currentBlock, "skip") &&
            block.currentBlock.skip !== undefined,
        );
      ...
      if (hasSkipBlock) {
        const skipConditions = [
          ...blocksForStep.flatMap((block) =>
            block.currentBlock.skip
              ? block.currentBlock.skip.map((condition) => ({
                  ...condition,
        ...
        skipConditions.some((condition) => {
          ...
          const validationCandidates = Object.fromEntries(
            Object.entries(answersWithNewAnswer).filter(([key]) => {
              const componentIndex = parseInt(
                key.slice(key.lastIndexOf("_") + 1),
                10,
              );
              return (
                componentIndex >= condition.firstIndex &&
                componentIndex <= currentStep
              );
            }),
          ...
          const componentsToCheck =
            condition.check !== "block"
              ? Object.entries(validationCandidates).filter(
                  ([key]) =>
                    key.slice(0, key.lastIndexOf("_")) === condition.name,
                )
          ...
          if (componentsToCheck.some(([_, responseObj]) => !responseObj)) {
            throw new Error(
              `There are components with missing response objects for the skip condition: ${JSON.stringify(condition, null, 2)}`,
            );
          }

          if (
            condition.check === "response" ||
            condition.check === "responses"
          ) {
            ...
            if (condition.check === "response") {
              ...
              // Check that the response is matches the correct answer
              conditionIsTriggered = !checkAllAnswersCorrect(
                response.answer,
                componentId,
                studyConfig.components[
                  componentId.slice(0, componentId.lastIndexOf("_"))
                ],
                studyConfig,
              );
            }
            ...
            condition.check === "block" ||
            condition.check === "repeatedComponent"
          ) {
            ...
            const correctAnswers = componentsToCheck.map(
              ([_componentName, responseObj]) =>
                checkAllAnswersCorrect(
                  responseObj.answer,
                  _componentName,
                  studyConfig.components[
                    componentName.slice(0, componentName.lastIndexOf("_"))
                  ],
                  studyConfig,
                ),
            );
            const numCorrect = correctAnswers.filter(
              (correct) => correct,
            ).length;
            ...
            conditionIsTriggered =
              (condition.condition === "numCorrect" &&
                numCorrect === condition.value) ||
              (condition.condition === "numIncorrect" &&
                numIncorrect === condition.value);
          }

          if (conditionIsTriggered) {
            const nextStepIndex = participantSequence.indexOf(condition.to);
            const nextStepBlockIndex =
              nextStepIndex === -1
                ? findIndexOfBlock(sequence, condition.to)
                : -1;
            nextStep =
              nextStepIndex === -1 ? nextStepBlockIndex : nextStepIndex;
            return true;
          }
          return false;
        });
      ...
      if (funcIndex) {
        navigate(
          `/${studyId}/${encryptIndex(currentStep)}/${encryptIndex(decryptIndex(funcIndex) + 1)}${window.location.search}`,
        );
      } else {
        navigate(
          `/${studyId}/${encryptIndex(nextStep)}${window.location.search}`,
        );
      }
    },
    [
