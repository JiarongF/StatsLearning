...
export function usePreviousStep() {
  ...
  const isPreviousDisabled =
    typeof currentStep !== "number" || isAnalysis || currentStep <= 0;
  ...
  const goToPreviousStep = useCallback(() => {
    ...
    if (funcIndex) {
      ...
      storeDispatch(
        deleteDynamicBlockAnswers({
          currentStep,
          funcIndex: decryptIndex(funcIndex),
    ...
    const isDynamicBlock = findFuncBlock(
      previousComponentId,
    ...
    if (isDynamicBlock) {
      ...
      const dynamicBlockAnswers = Object.keys(answers).filter((key) =>
        key.startsWith(`${previousComponentId}_${previousStep}_`),
      );
      const previousDynamicBlockIndex = dynamicBlockAnswers.length - 1;
      // Navigate to the last answered index in the dynamic block
      navigate(
        `/${studyId}/${encryptIndex(previousStep)}/${encryptIndex(previousDynamicBlockIndex)}${window.location.search}`,
      );
    } else {
      navigate(
        `/${studyId}/${encryptIndex(previousStep)}${window.location.search}`,
      );
    }
  }, [
    currentStep,
