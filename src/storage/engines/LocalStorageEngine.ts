...
export class LocalStorageEngine extends StorageEngine {
  ...
  protected async _getFromStorage<T extends StorageObjectType>(
    ...
    const storedObject =
      await this.studyDatabase.getItem<StorageObject<T>>(storageKey);
    return storedObject;
  }
  ...
  protected async _getAllSequenceAssignments(studyId: string) {
    ...
    const sequenceAssignments = await this.studyDatabase.getItem<
      Record<string, SequenceAssignment>
    >(sequenceAssignmentPath);
    if (!sequenceAssignments) {
      return [];
    }
    return Object.values(sequenceAssignments).sort(
      (a, b) => a.timestamp - b.timestamp,
    );
  }

  protected async _createSequenceAssignment(
    ...
    const sequenceAssignments =
      (await this.studyDatabase.getItem<Record<string, SequenceAssignment>>(
        sequenceAssignmentPath,
      )) || {};
    sequenceAssignments[participantId] = sequenceAssignment;
    await this.studyDatabase.setItem(
      sequenceAssignmentPath,
      sequenceAssignments,
    );
  }
  ...
  protected async _rejectParticipantRealtime(participantId: string) {
    ...
    const claimedAssignmentData = Object.values(sequenceAssignments).find(
      (assignment) =>
        assignment.claimed &&
  ...
  protected async _claimSequenceAssignment(
    participantId: string,
    sequenceAssignment: SequenceAssignment,
  ) {
    await this.verifyStudyDatabase();
    ...
    if (sequenceAssignments[participantId]) {
      ...
      throw new Error(
        `Sequence assignment for participant ${participantId} not found`,
      );
    }
  }
  ...
  async getModes(studyId: string) {
    ...
    // Get the modes
    const modes = (await this.studyDatabase.getItem(key)) as Record<
      REVISIT_MODE,
      boolean
    > | null;
    if (modes) {
  ...
  protected async _getAudioUrl(task: string, participantId?: string) {
    ...
    const audioBlob = await this._getFromStorage(
      `audio/${participantId || this.currentParticipantId}`,
      task,
    );
    if (!audioBlob) {
      throw new Error(
        `Audio for task ${task} and participant ${participantId || this.currentParticipantId} not found`,
      );
    }
  ...
  protected async _testingReset(studyId: string) {
    ...
    const studyKeys = keys.filter((key) =>
      key.startsWith(`${this.collectionPrefix}${studyId}/`),
    );
    await Promise.all(
      studyKeys.map((key) => this.studyDatabase.removeItem(key)),
    );
  ...
  async getSnapshots(studyId: string) {
    ...
    const snapshotsData =
      await this.studyDatabase.getItem<SnapshotDocContent>(snapshotsKey);
  ...
  protected async _copyDirectory(source: string, target: string) {
    ...
    const copyPromises = sourceKeys.map(async (key) => {
      if (
        key.endsWith("/snapshots") ||
        key.endsWith("modes") ||
        key.endsWith("configHash") ||
        key.endsWith("currentParticipantId")
      ) {
  ...
  protected async _deleteDirectory(path: string) {
    ...
    const targetKeys = keys.filter(
      (key) => key.startsWith(path) && !key.includes("snapshots"),
    );
    const deletePromises = targetKeys.map((key) =>
      this.studyDatabase.removeItem(key),
    );
  ...
  protected async _addDirectoryNameToSnapshots(
    ...
    const metadata =
      (await this.studyDatabase.getItem<SnapshotDocContent>(metadataKey)) || {};
  ...
  protected async _removeDirectoryNameFromSnapshots(
    directoryName: string,
    studyId: string,
  ) {
    await this.verifyStudyDatabase();
  ...
  protected async _changeDirectoryNameInSnapshots(
    key: string,
    newName: string,
    studyId: string,
  ) {
