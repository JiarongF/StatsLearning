...
export class SupabaseStorageEngine extends StorageEngine {
  ...
  protected async _getFromStorage<T extends StorageObjectType>(
    ...
    const dataToParse = this.testing
      ? new Blob([data as unknown as string], { type: "application/json" })
      : data;
    try {
  ...
  protected async _pushToStorage<T extends StorageObjectType>(
    ...
    if (objectToUpload instanceof Blob) {
      ...
      uploadObject = this.testing
        ? Buffer.from(JSON.stringify(objectToUpload))
        : objectToUpload;
    } else {
    ...
    return data
      .map(
        (item) =>
          ({
            ...item.data,
            timestamp: item.data.withServerTimestamp
  ...
  protected async _completeCurrentParticipantRealtime() {
    ...
    if (error || !data) {
      throw new Error(
        "Failed to retrieve sequence assignment for current participant",
      );
    }
  ...
  protected async _rejectParticipantRealtime(participantId: string) {
    ...
    await this.supabase
      .from("revisit")
      .update({
        data: { ...data.data, rejected: true, timestamp: new Date().getTime() },
      })
      .eq("studyId", `${this.collectionPrefix}${this.studyId}`)
      .eq("docId", sequenceAssignmentPath);
    ...
    if (claimedError || !claimedData) {
      throw new Error(
        "Failed to retrieve claimed sequence assignment for rejection",
      );
    }
  ...
  async initializeStudyDb(studyId: string) {
    ...
    const { error: schemaError } = await this.supabase.from("revisit").upsert({
  ...
  protected async _copyDirectory(source: string, target: string) {
    ...
    const copyPromises = keys.map(async (key) => {
      ...
      if (uploadError) {
        console.error(
          `Error uploading file to ${target}/${key.name}:`,
          uploadError,
        );
      }
    });
  ...
  protected async _addDirectoryNameToSnapshots(
    ...
    if (!snapshots[directoryName]) {
      ...
      await this.supabase.from("revisit").upsert({
        studyId: `${this.collectionPrefix}${studyId}`,
        docId: "snapshots",
        data: snapshots,
      });
