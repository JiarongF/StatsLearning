...
describe.each([
  ...
  test("_pushToStorage, _getFromStorage, and _removeFromStorage work correctly", async () => {
    ...
    const participantData = await storageEngine.initializeParticipantSession(
      {},
      configSimple,
      participantMetadata,
    );
    ...
    // @ts-expect-error using protected method for testing
    const storedSequenceArray = await storageEngine._getFromStorage(
      "",
      "sequenceArray",
    );
    ...
    // @ts-expect-error using protected method for testing
    const storedParticipantData = await storageEngine._getFromStorage(
      "participants",
      "participantData",
      studyId,
    );
    expect(storedParticipantData).toBeDefined();
    expect(storedParticipantData!.participantId).toBe(
  ...
  test("_verifyStudyDatabase throws error if not initialized", async () => {
    ...
    // @ts-expect-error using protected method for testing
    await expect(
      uninitializedStorageEngine.verifyStudyDatabase(),
  ...
  test("_getAllSequenceAssignments returns sequence assignment for participant", async () => {
    ...
    expect(sequenceAssignment!.createdTime).equal(
      sequenceAssignment!.timestamp,
    );
  });
  ...
  test("_completeCurrentParticipantRealtime updates sequence assignment", async () => {
    ...
    const updatedSequenceAssignment = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId,
    );
    ...
    expect(updatedSequenceAssignment!.completed).toBeGreaterThanOrEqual(
      updatedSequenceAssignment!.createdTime,
    );
  });
  ...
  test("_getSnapshotData is empty on initialization", async () => {
    ...
    const snapshotData = await storageEngine.getSnapshots(
      `${storageEngine.collectionPrefix}${studyId}`,
    );
  ...
  test("_directoryExists returns true for existing directory", async () => {
    ...
    const exists = await storageEngine._directoryExists(
      `${storageEngine.collectionPrefix}${studyId}`,
    );
  ...
  test("_copyRealtimeData copies realtime data", async () => {
    ...
    // @ts-expect-error using protected method for testing
    const sourceSequenceAssignments1 =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sourceSequenceAssignments1).toBeDefined();
    expect(sourceSequenceAssignments1.length).toEqual(1);

    // @ts-expect-error using protected method for testing
    let targetSequenceAssignments =
      await storageEngine._getAllSequenceAssignments(targetId);
    ...
    // @ts-expect-error using protected method for testing
    const sourceSequenceAssignments2 =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sourceSequenceAssignments2).toBeDefined();
    expect(sourceSequenceAssignments2.length).toEqual(1);

    // @ts-expect-error using protected method for testing
    targetSequenceAssignments =
      await storageEngine._getAllSequenceAssignments(targetId);
    expect(targetSequenceAssignments).toBeDefined();
    expect(targetSequenceAssignments.length).toEqual(1);
    expect(targetSequenceAssignments[0].participantId).toBe(
      sourceSequenceAssignments2[0].participantId,
    );
  ...
  test("_addDirectoryNameToSnapshots, _removeDirectoryNameFromSnapshots, and _changeDirectoryNameInSnapshots work correctly", async () => {
    ...
    // @ts-expect-error using protected method for testing
    const snapshotData1 = await storageEngine.getSnapshots(
      `${storageEngine.collectionPrefix}${studyId}`,
    );
    ...
    // @ts-expect-error using protected method for testing
    await storageEngine._changeDirectoryNameInSnapshots(
      directoryName,
      newDirectoryName,
      studyId,
    );
    ...
    // @ts-expect-error using protected method for testing
    await storageEngine._removeDirectoryNameFromSnapshots(
      directoryName,
      studyId,
    );
