...
describe.each([
  ...
  test("saveConfig and getAllConfigsFromHash work correctly", async () => {
    ...
    let storedHashes = await storageEngine.getAllConfigsFromHash(
      [configHash],
      studyId,
    );
    ...
    storedHashes = await storageEngine.getAllConfigsFromHash(
      [configHash, configComplexHash],
      studyId,
    );
  ...
  test("initializeParticipantSession makes data", async () => {
    ...
    const participantData =
      await storageEngine.getParticipantData(participantId);
    ...
    expect(participantData!.participantId).toBe(
      participantSession.participantId,
    );
    expect(participantData!.participantConfigHash).toEqual(
  ...
  test("initializeParticipantSession with urlParticipantId", async () => {
    ...
    const participantData =
      await storageEngine.getParticipantData(urlParticipantId);
  ...
  test("rejectParticipant on participants allows ", async () => {
    ...
    await storageEngine.rejectParticipant(
      participantId1,
      "Participant rejected for testing",
    );

    // @ts-expect-error using protected method for testing
    let sequenceAssignments =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sequenceAssignments).toBeDefined();

    let sequenceAssignment1 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId1,
    );
    ...
    let sequenceAssignment2 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId2,
    );
    ...
    expect(sequenceAssignment2!.timestamp).toEqual(
      sequenceAssignment1!.timestamp,
    );
    ...
    expect(sequenceAssignment2!.createdTime).toBeGreaterThanOrEqual(
      sequenceAssignment1!.createdTime,
    );
    ...
    sequenceAssignments =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sequenceAssignments).toBeDefined();
    sequenceAssignment2 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId2,
    );
    expect(sequenceAssignment2).toBeDefined();
    expect(sequenceAssignment2!.timestamp).toBeGreaterThanOrEqual(
      sequenceAssignment1!.timestamp,
    );

    // Check if the first rejected participant's sequence is available for reuse
    sequenceAssignment1 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId1,
    );
    expect(sequenceAssignment1).toBeDefined();
    expect(sequenceAssignment1!.rejected).toBe(true);
    ...
    await storageEngine.clearCurrentParticipantId();
    const { participantId: participantId3 } =
      await storageEngine.initializeParticipantSession(
        {},
        configSimple,
        participantMetadata,
    ...
    sequenceAssignments =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sequenceAssignments).toBeDefined();
    const sequenceAssignment3 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId3,
    );
    expect(sequenceAssignment3).toBeDefined();
    expect(sequenceAssignment3!.participantId).toBe(participantId3);
    expect(sequenceAssignment3!.timestamp).toEqual(
      sequenceAssignment1!.timestamp,
    );
    ...
    expect(sequenceAssignment3!.createdTime).toBeGreaterThan(
      sequenceAssignment1!.createdTime,
    );
    ...
    expect(sequenceAssignment1).toBeDefined();
    expect(sequenceAssignment1!.participantId).toBe(participantId1);
    expect(sequenceAssignment1!.timestamp).toEqual(
      sequenceAssignment3!.timestamp,
    );
    expect(sequenceAssignment1!.rejected).toBe(true);
    expect(sequenceAssignment1!.claimed).toBe(true);
    expect(sequenceAssignment1!.createdTime).toBeDefined();
    expect(sequenceAssignment1!.createdTime).toBeLessThanOrEqual(
      sequenceAssignment3!.createdTime,
    );

    // Check that a new participant gets the other rejected sequence assignment
    await storageEngine.clearCurrentParticipantId();
    const { participantId: participantId4 } =
      await storageEngine.initializeParticipantSession(
        {},
        configSimple,
        participantMetadata,
    ...
    sequenceAssignments =
      await storageEngine._getAllSequenceAssignments(studyId);
    expect(sequenceAssignments).toBeDefined();
    const sequenceAssignment4 = sequenceAssignments.find(
      (assignment) => assignment.participantId === participantId4,
    );
    ...
    expect(sequenceAssignment4!.createdTime).toBeGreaterThanOrEqual(
      sequenceAssignment3!.createdTime,
    );
  ...
  test("setSequenceArray throws error if not connected", async () => {
    ...
    await expect(
      disconnectedStorageEngine.setSequenceArray([]),
    ).rejects.toThrow("Study database not initialized");
  });
  ...
  test("createSnapshot, restoreSnapshot, renameSnapshot, removeSnapshotOrLive, getSnapshots work correctly", async () => {
    const actionResponse1 = await storageEngine.createSnapshot(
      "non-existent",
      false,
    );
    ...
    expect(actionResponse1.error!.message).toBe(
    ...
    expect(actionResponse2.notifications![0].message).toBe(
    ...
    const renameResponse = await storageEngine.renameSnapshot(
      snapshotKey,
      newSnapshotName,
      studyId,
    );
    ...
    expect(renameResponse.notifications![0].message).toBe(
      "Successfully renamed snapshot.",
    );
    expect(renameResponse.notifications![0].color).toBe("green");

    // Remove the snapshot
    const removeResponse = await storageEngine.removeSnapshotOrLive(
    ...
    expect(removeResponse.notifications![0].message).toBe(
      "Successfully deleted snapshot or live data.",
    );
    ...
    expect(actionResponse3.notifications![0].message).toBe(
      "Successfully deleted live data.",
    );
    expect(actionResponse3.notifications![0].color).toBe("green");
    expect(actionResponse3.notifications![1].title).toBe("Success!");
    expect(actionResponse3.notifications![1].message).toBe(
      "Successfully created snapshot",
    );
    ...
    const restoreResponse = await storageEngine.restoreSnapshot(
      studyId,
      snapshotName,
    );
    ...
    expect(restoreResponse.notifications![0].title).toBe(
      "Failed to Create Snapshot.",
    );
    expect(restoreResponse.notifications![0].message).toBe(
      "There is currently no data in your study. A snapshot could not be created.",
    );
    expect(restoreResponse.notifications![0].color).toBe("yellow");
    expect(restoreResponse.notifications![1].title).toBe("Success!");
    expect(restoreResponse.notifications![1].message).toBe(
      "Successfully restored snapshot to live data.",
    );
