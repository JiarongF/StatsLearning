...
import {
  expandLibrarySequences,
  loadLibrariesParseNamespace,
  verifyLibraryUsage,
} from "./libraryParser";
...
const globalValidate = ajv1.getSchema<GlobalConfig>(
...
export function parseGlobalConfig(fileData: string) {
  ...
  console.error("Global config parsing errors", [
    ...(globalValidate.errors || []),
    ...extraValidation[1],
  ]);
...
function verifyStudySkip(
  ...
  if (sequence.id) {
    ...
    const idxToRemove = skipTargets.map((target, idx) => {
...
function verifyStudyConfig(
  ...
  Object.entries(studyConfig.components).forEach(
    ([componentName, component]) => {
      ...
      if (
        ...
        errors.push({
          ...
          params: {
            action: "add the base component to the baseComponents object",
          },
        });
      }
    },
  ...
  const usedComponents = getSequenceFlatMapWithInterruptions(
  ...
  Object.keys(studyConfig.components)
    ...
    .forEach((componentName) => {
      warnings.push({
        ...
        params: {
          action:
            "remove the component from the components object or add it to the sequence",
        },
      });
    });
...
export async function parseStudyConfig(
  fileData: string,
): Promise<ParsedConfig<StudyConfig>> {
  ...
  if (validatedData && data) {
    ...
    const importedLibrariesData = await loadLibrariesParseNamespace(
      importedLibraries,
      errors,
      warnings,
    );
    ...
    data.sequence = expandLibrarySequences(
      data.sequence,
      importedLibrariesData,
      errors,
    );

    const { errors: parserErrors, warnings: parserWarnings } =
      verifyStudyConfig(data, importedLibrariesData);
    ...
    errors = [
      ...errors,
      {
        ...
        params: {
          action:
            "fix the errors in your file or make sure the global config references the right file path",
        },
      },
    ];
  }
