...
const libraryValidate = ajv.getSchema<LibraryConfig>(
  "#/definitions/LibraryConfig",
)!;
  ...
  return {
    ...
    components: (sequence.components || []).map((component) => {
      ...
      if (
        ...
        librarySequence = namespaceLibrarySequenceComponents(
          librarySequence,
          cleanLibraryName,
        );
...
export function verifyLibraryUsage(
  ...
  Object.entries(importedLibrariesData).forEach(([library, libraryData]) => {
    ...
    Object.entries(libraryData.components).forEach(
...
async function getLibraryConfig(libraryName: string) {
  const config = await (
    await fetch(`${PREFIX}libraries/${libraryName}/config.json`)
  ).text();
...
export async function loadLibrariesParseNamespace(
  ...
  const importedLibrariesData: Record<
    string,
    ParsedConfig<LibraryConfig>
  > = Object.fromEntries(await Promise.all(loadedLibraries));

  // Filter out the missing imported libraries
  Object.entries(importedLibrariesData).forEach(
        ...
        .map(([componentName, component]) => {
          if (isInheritedComponent(component)) {
            const mergedComponent = merge(
              {},
              importedLibrariesData[libraryName].baseComponents![
