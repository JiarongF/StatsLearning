...
export function getSequenceFlatMap<
  T extends Sequence | StudyConfig["sequence"],
>(sequence: T): string[] {
  return isDynamicBlock(sequence)
    ...
    : sequence.components.flatMap((component) =>
        typeof component === "string"
          ? component
          : getSequenceFlatMap(component),
      );
}

export function findAllFuncBlocks(
  sequence: StudyConfig["sequence"],
): DynamicBlock[] {
  return isDynamicBlock(sequence)
    ? [sequence]
    : sequence.components.flatMap((component) =>
        typeof component === "string" ? [] : findAllFuncBlocks(component),
      );
}
...
export function getSequenceFlatMapWithInterruptions(
  sequence: StudyConfig["sequence"],
): string[] {
  ...
  return [
    ...sequence.components.flatMap((component) =>
      typeof component === "string"
        ? component
        : isDynamicBlock(component)
          ? []
          : getSequenceFlatMapWithInterruptions(component),
    ),
    ...(sequence.interruptions?.flatMap(
...
function _findBlockForStep(
  sequence: Sequence,
  pathOrStep: string | number,
  distance: number,
): SkipConditionWithExtents[] | number {
  let componentsSeen = 0;
  for (let i = 0; i < sequence.components.length; i += 1) {
    const component = sequence.components[i];
    if (typeof component === "string") {
      if (
        typeof pathOrStep === "number" &&
        pathOrStep === distance + componentsSeen
      ) {
        return [
          {
            currentBlock: sequence,
      ...
      const result = _findBlockForStep(
        component,
        pathOrStep,
        distance + componentsSeen,
      );
...
export function findBlockForStep(
  sequence: Sequence,
  pathOrStep: string | number,
) {
...
function _findIndexOfBlock(
  ...
  for (let i = 0; i < sequence.components.length; i += 1) {
    ...
    if (typeof component === "string") {
      ...
      const result = _findIndexOfBlock(
        component,
        to,
        distance + componentsSeen,
      );
...
export function configSequenceToUniqueTrials(
  sequence: ComponentBlock | DynamicBlock,
  orderPath = "root",
): { componentName: string; orderPath: string; timesSeenInBlock: number }[] {
  if (isDynamicBlock(sequence)) {
    return [];
  }
  ...
  sequence.components.forEach((component, index) => {
    if (typeof component === "string") {
      result.push({
        ...
        orderPath,
        timesSeenInBlock: componentsSeen[component] || 0,
      });
      componentsSeen[component] = componentsSeen[component]
        ? componentsSeen[component] + 1
        : 1;
    } else {
      result.push(
        ...configSequenceToUniqueTrials(component, `${orderPath}-${index}`),
      );
    }
  });
  return result;
...
export function addPathToComponentBlock(
  order: StudyConfig["sequence"] | ComponentBlockWithOrderPath | string,
  orderPath: string,
): ComponentBlockWithOrderPath | string {
  ...
  return {
    ...
    orderPath,
    order: order.order,
    components: order.components.map((o, i) =>
      addPathToComponentBlock(o, `${orderPath}-${i}`),
    ),
  };
}
