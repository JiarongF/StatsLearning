...
function StudyCard({
  ...
  const [studyStatusAndTiming, setStudyStatusAndTiming] = useState<{
    completed: number;
    rejected: number;
    inProgress: number;
    minTime: Timestamp | number | null;
    maxTime: Timestamp | number | null;
  } | null>(null);
  useEffect(() => {
  ...
  const { minTime, maxTime } = useMemo(() => {
    ...
    if (!studyStatusAndTiming.minTime || !studyStatusAndTiming.maxTime)
      return { minTime: null, maxTime: null };

    const min =
      typeof studyStatusAndTiming.minTime === "number"
        ? new Date(studyStatusAndTiming.minTime)
        : studyStatusAndTiming.minTime.toDate();
    const max =
      typeof studyStatusAndTiming.maxTime === "number"
        ? new Date(studyStatusAndTiming.maxTime)
        : studyStatusAndTiming.maxTime.toDate();
  ...
  const [modes, setModes] = useState<Record<REVISIT_MODE, boolean> | null>(
    null,
  );
  useEffect(() => {
    if (!storageEngine) return;
          ...
          {config.warnings.length > 0 && (
            <>
              <Flex align="center" direction="row">
                ...
                <Text fw="bold" ml={8} color="orange">
                  Warnings
                </Text>
              </Flex>
          ...
          <Flex direction="row" gap="sm">
            ...
            <Flex ml="auto" gap="sm" opacity={0.7}>
              {modes?.studyNavigatorEnabled ? (
                <Tooltip label="Study navigator enabled" withinPortal>
                  <IconSchema size={16} color="green" />
                </Tooltip>
              ) : (
                <Tooltip label="Study navigator disabled" withinPortal>
                  <IconSchemaOff size={16} color="red" />
                </Tooltip>
              )}
              ...
              {storageEngine?.getEngine() === "localStorage" ? (
                <Tooltip label="Local storage enabled" withinPortal>
                  <IconDatabase size={16} color="green" />
                </Tooltip>
              ) : (
                <Tooltip label="Firebase enabled" withinPortal>
...
export function ConfigSwitcher({
  ...
  const [studyVisibility, setStudyVisibility] = useState<
  ...
  const configsFiltered = useMemo(
    () =>
      configsList.filter(
        (configName) => studyVisibility[configName] || user.isAdmin,
      ),
    [configsList, studyVisibility, user],
  );

  const demos = useMemo(
    () =>
      configsFiltered.filter((configName) => configName.startsWith("demo-")),
    [configsFiltered],
  );
  const tutorials = useMemo(
    () =>
      configsFiltered.filter((configName) => configName.startsWith("tutorial")),
    [configsFiltered],
  );
  const examples = useMemo(
    () =>
      configsFiltered.filter((configName) => configName.startsWith("example-")),
    [configsFiltered],
  );
  const tests = useMemo(
    () =>
      configsFiltered.filter((configName) => configName.startsWith("test-")),
    [configsFiltered],
  );
  const libraries = useMemo(
    () =>
      configsFiltered.filter((configName) => configName.startsWith("library-")),
    [configsFiltered],
  );
  const others = useMemo(
    () =>
      configsFiltered.filter(
        (configName) =>
  ...
  const tab = useMemo(
    () => searchParams.get("tab") || (others.length > 0 ? "Others" : "Demos"),
    [others.length, searchParams],
  );
  ...
  return (
    <AppShell.Main>
      <Container size="sm" px={0}>
        ...
        <Tabs
          variant="outline"
          defaultValue={others.length > 0 ? "Others" : "Demos"}
          value={tab}
        ...
        >
          ...
          <Tabs.Panel value="Demos">
            <Text c="dimmed" mt="sm">
              These studies show off individual features of the reVISit
              platform.
            </Text>
          ...
          <Tabs.Panel value="Examples">
            <Text c="dimmed" mt="sm">
              These are full studies that demonstrate the capabilities of the
              reVISit platform.
            </Text>
          ...
          <Tabs.Panel value="Tutorials">
            <Text c="dimmed" mt="sm">
              These studies are designed to help you learn how to use the
              reVISit platform.
            </Text>
          ...
          <Tabs.Panel value="Tests">
            <Text c="dimmed" mt="sm">
              These studies exist for testing purposes.
            </Text>
          ...
          <Tabs.Panel value="Libraries">
            <Text c="dimmed" mt="sm">
              Here you can see an example of every library that we publish.
            </Text>
