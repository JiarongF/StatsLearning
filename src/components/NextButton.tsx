...
export function NextButton({
  ...
  const nextButtonDisableTime = useMemo(
    () =>
      configInUse?.nextButtonDisableTime ??
      studyConfig.uiConfig.nextButtonDisableTime,
    [configInUse, studyConfig],
  );
  const nextButtonEnableTime = useMemo(
    () =>
      configInUse?.nextButtonEnableTime ??
      studyConfig.uiConfig.nextButtonEnableTime ??
      0,
    [configInUse, studyConfig],
  );
  ...
  const buttonTimerSatisfied = useMemo(() => {
    const nextButtonDisableSatisfied =
      nextButtonDisableTime && timer ? timer <= nextButtonDisableTime : true;
    const nextButtonEnableSatisfied = timer
      ? timer >= nextButtonEnableTime
      : true;
  ...
  const previousButtonText = useMemo(
    () =>
      configInUse?.previousButtonText ??
      studyConfig.uiConfig.previousButtonText ??
      "Previous",
    [configInUse, studyConfig],
  );
        ...
        <>
          {nextButtonEnableTime > 0 && timer < nextButtonEnableTime && (
            ...
            >
              ...
              {Math.ceil((nextButtonEnableTime - timer) / 1000)} seconds.
            </Alert>
          )}
          {nextButtonDisableTime &&
            timer &&
            ...
            (nextButtonDisableTime - timer > 0 ? (
              ...
              >
                ...
                {Math.ceil((nextButtonDisableTime - timer) / 1000)} seconds.
              </Alert>
            ) : (
              !studyConfig.uiConfig.timeoutReject && (
                <Alert
