...
export function WithinTaskProvenance({
  ...
  const colorMap = useMemo(() => {
    ...
    if (answer.provenanceGraph) {
      let idx = 0;
      Object.entries(answer.provenanceGraph.stimulus?.nodes || {}).forEach(
        (entry) => {
          const [, node] = entry;
  ...
  return (
    <g style={{ cursor: "pointer" }}>
      {provenance
        ? Object.entries(provenance.nodes || {}).map((entry) => {
            const [nodeId, node] = entry;
            return (
              <g key={nodeId}>
                <rect
                  fill={colorMap.get(node.label) || "#9498a0"}
      ...
      {currentNode && provenance && provenance.nodes[currentNode] && (
        <rect
          fill={colorMap.get(provenance.nodes[currentNode].label) || "#9498a0"}
