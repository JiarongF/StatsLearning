...
import { Registry, Trrack, initializeTrrack, isRootNode } from "@trrack/core";
...
export function AudioProvenanceVis({
  ...
  const analysisIsPlaying = useStoreSelector(
    (state) => state.analysisIsPlaying,
  );
  const analysisHasAudio = useStoreSelector((state) => state.analysisHasAudio);

  const { saveAnalysisState, setAnalysisHasAudio, setAnalysisIsPlaying } =
    useStoreActions();
  ...
  }, [currentStep, saveAnalysisState, storeDispatch]);

  const [ref, { width }] = useResizeObserver();
  const [waveSurferWidth, setWaveSurferWidth] = useState<number>(0);

  const [currentNode, setCurrentNode] = useState<string | null>(null);
  const [currentResponseNodes, setCurrentResponseNodes] = useState<
    Record<ResponseBlockLocation, string | undefined>
  >({
  ...
  const [currentGlobalNode, setCurrentGlobalNode] = useState<{
    name: string;
    time: number;
  } | null>(null);
  ...
  const _setCurrentResponseNodes = useEvent(
    (node: string | null, location: ResponseBlockLocation) => {
      const graph = answers[identifier]?.provenanceGraph[location];
      if (graph && node) {
        if (
          !currentGlobalNode ||
          graph.nodes[node].createdOn > currentGlobalNode.time ||
          playTime < currentGlobalNode.time
        ) {
          setCurrentGlobalNode({
            name: node || "",
  ...
  useUpdateProvenance(
    "aboveStimulus",
    playTime,
    answers[identifier]?.provenanceGraph.aboveStimulus,
    currentResponseNodes.aboveStimulus,
    _setCurrentResponseNodes,
  );

  useUpdateProvenance(
    "belowStimulus",
    playTime,
    answers[identifier]?.provenanceGraph.belowStimulus,
    currentResponseNodes.belowStimulus,
    _setCurrentResponseNodes,
  );
  ...
  useUpdateProvenance(
    "sidebar",
    playTime,
    answers[identifier]?.provenanceGraph.sidebar,
    currentResponseNodes.sidebar,
    _setCurrentResponseNodes,
  );
  ...
  useEffect(() => {
    if (identifier && answers[identifier]?.provenanceGraph) {
      ...
      if (answers[identifier]?.provenanceGraph.stimulus) {
        trrack.importObject(
          structuredClone(answers[identifier]?.provenanceGraph!.stimulus),
        );
  ...
  const _setCurrentNode = useCallback(
    (node: string | undefined) => {
      ...
      if (identifier && trrackForTrial.current) {
        storeDispatch(
          saveAnalysisState({
            prov: trrackForTrial.current.getState(
              answers[identifier]?.provenanceGraph.stimulus?.nodes[node],
            ),
            location: "stimulus",
          }),
        );
  ...
  useEffect(() => {
    if (
      !identifier ||
      !trrackForTrial.current ||
      !answers[identifier]?.provenanceGraph
    ) {
      return;
    }
  ...
  const startTime = useMemo(
    () => answers[identifier]?.startTime || 0,
    [answers, identifier],
  );
  ...
  useEffect(() => {
    if (totalAudioLength === 0) {
      ...
      setTimeString(
        `${humanReadableDuration(playTime - startTime)} / ${humanReadableDuration(totalAudioLength * 1000)}`,
      );
    }
  ...
  useEffect(() => {
    if (!analysisHasAudio) {
      ...
      const length =
        answers[identifier]?.endTime - answers[identifier]?.startTime;
  ...
  const _setPlayTime = useThrottledCallback(
    (n: number, percent: number | undefined) => {
  ...
  const xScale = useMemo(() => {
    ...
    const endTime =
      totalAudioLength > 0
        ? answers[identifier]?.startTime + totalAudioLength * 1000
        : answers[identifier]?.endTime;

    const scale = d3
      .scaleLinear([margin.left, width + margin.left + margin.right])
      .domain([answers[identifier]?.startTime, endTime])
      .clamp(true);
