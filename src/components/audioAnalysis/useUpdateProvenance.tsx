...
export function useUpdateProvenance(
  ...
  playTime: number,
  provGraph: TrrackedProvenance | undefined,
  currentNode: string | undefined,
  setCurrentNode: (
    node: string | null,
    _location: ResponseBlockLocation,
  ) => void,
) {
  ...
  useEffect(() => {
    ...
    if (!currentNode || !provGraph.nodes[currentNode]) {
      ...
      storeDispatch(
        saveAnalysisState({
          prov: trrackInstance.getState(provGraph.nodes[provGraph.root]),
          location,
        }),
      );
    ...
    if (tempNode.id !== currentNode) {
      ...
      storeDispatch(
        saveAnalysisState({
          prov: trrackInstance.getState(tempNode),
          location,
        }),
      );
    }
