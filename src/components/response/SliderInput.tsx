import { Box, Flex, Input, Slider, SliderProps } from "@mantine/core";
...
export function SliderInput({
  ...
  const [min, max] = useMemo(
    () => [
      Math.min(...options.map((opt) => opt.value)),
      ...
      <Slider
        ...
        styles={(theme) => ({
          ...
          markLabel: {
            ...
            transform:
              "translate(calc((var(--mark-offset) * -1) + (var(--slider-size) / 2)), calc(var(--mantine-spacing-xs) / 2)",
          },
