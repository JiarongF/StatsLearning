...
export function ResponseSwitcher({
  ...
  const ans = (isAnalysis ? { value: storedAnswer![response.id] } : form) || {
    value: undefined,
  };
  const dontKnowValue = (Object.keys(storedAnswer || {}).length > 0
    ? { checked: storedAnswer![`${response.id}-dontKnow`] }
    : dontKnowCheckbox) || { checked: undefined };
  const otherValue = (Object.keys(storedAnswer || {}).length > 0
    ? { value: storedAnswer![`${response.id}-other`] }
  ...
  const enumerateQuestions = useMemo(
    () =>
      configInUse?.enumerateQuestions ??
  ...
  const isDisabled = useMemo(() => {
      ...
      for (let i = 0; i < sequence.components.length; i += 1) {
        ...
        if (typeof component === "string") {
          if (component === currentComponent) {
            ...
            if (
              i + 1 < sequence.components.length &&
              typeof sequence.components[i + 1] !== "string"
            ) {
              return false;
            }
            break;
    ...
    if (
      ...
      currentStep + 1 < flatSequence.length
    ) {
  ...
  const fieldInitialValue = useMemo(() => {
    ...
    response.paramCapture,
    (response as MatrixResponse).questionOptions,
    (response as SliderResponse).startingValue,
    response.type,
    searchParams,
  ]);

  const responseStyle = response.style || {};
  const responseDividers = useMemo(
    () =>
      response.withDivider ??
      configInUse?.responseDividers ??
      studyConfig.uiConfig.responseDividers,
    [response, configInUse, studyConfig],
  );
  ...
  return (
    ...
    >
      ...
      {response.type === "textOnly" && <TextOnlyInput response={response} />}
      {response.withDontKnow && (
        <Checkbox
          ...
          onChange={(event) => {
            dontKnowCheckbox?.onChange(event.currentTarget.checked);
