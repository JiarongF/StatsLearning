import { Alert, Anchor, Box, Button } from "@mantine/core";
...
export function ResponseBlock({ config, location, status, style }: Props) {
  ...
  const currentProvenance = useStoreSelector(
    (state) => state.analysisProvState[location],
  ) as FormElementProvenance | undefined;

  const storedAnswer = useMemo(
    () => currentProvenance?.form || status?.answer,
    [currentProvenance, status],
  );
  const storedAnswerData = useStoredAnswer();
  const formOrders: Record<string, string[]> = useMemo(
    () => storedAnswerData?.formOrder || {},
    [storedAnswerData],
  );
  ...
  const responsesWithDefaults = useMemo(
    () =>
      responses.map((response) => {
        if (response.type !== "textOnly") {
          return {
            ...response,
            required:
              response.required === undefined ? true : response.required,
          };
        }
        return response;
      }),
  ...
  const { actions, trrack } = useMemo(() => {
    ...
    const updateFormAction = reg.register(
      "update",
      (state, payload: StoredAnswer["answer"]) => {
        state.form = payload;
        return state;
      },
    );
  ...
  const provideFeedback = useMemo(
    () => configInUse?.provideFeedback ?? studyConfig.uiConfig.provideFeedback,
    [configInUse, studyConfig],
  );
  const hasCorrectAnswerFeedback =
    provideFeedback && (configInUse?.correctAnswer?.length || 0) > 0;
  const allowFailedTraining = useMemo(
    () =>
      configInUse?.allowFailedTraining ??
      studyConfig.uiConfig.allowFailedTraining ??
      true,
    [configInUse, studyConfig],
  );
  const [attemptsUsed, setAttemptsUsed] = useState(0);
  const trainingAttempts = useMemo(
    () =>
      configInUse?.trainingAttempts ??
      studyConfig.uiConfig.trainingAttempts ??
      2,
    [configInUse, studyConfig],
  );
  ...
  const usedAllAttempts =
    attemptsUsed >= trainingAttempts && trainingAttempts >= 0;
  const disabledAttempts = usedAllAttempts || hasCorrectAnswer;
  const showBtnsInLocation = useMemo(
    () =>
      location ===
      (configInUse?.nextButtonLocation ??
        studyConfig.uiConfig.nextButtonLocation ??
        "belowStimulus"),
    [configInUse, studyConfig, location],
  );
  const identifier = useCurrentIdentifier();

  useEffect(() => {
    const ReactiveResponse = responsesWithDefaults.find(
      (r) => r.type === "reactive",
    );
    if (reactiveAnswers && ReactiveResponse) {
      ...
      answerValidator.setValues({
        ...
        [answerId]: reactiveAnswers[answerId] as string[],
      });
    }
  ...
  useEffect(() => {
    ...
    const matrixResponse = responsesWithDefaults.filter(
      (r) => r.type === "matrix-radio" || r.type === "matrix-checkbox",
    );
  ...
  useEffect(() => {
    trrack.apply(
      "update",
      actions.updateFormAction(structuredClone(answerValidator.values)),
    );
  ...
  const checkAnswerProvideFeedback = useCallback(() => {
    ...
    const correctAnswers = Object.fromEntries(
      responsesWithDefaults.map((response) => {
        const configCorrectAnswer = configInUse?.correctAnswer?.find(
          (answer) => answer.id === response.id,
        )?.answer;
        const suppliedAnswer = (
          answerValidator.values as Record<string, unknown>
        )[response.id];

        return [
          response.id,
          Array.isArray(suppliedAnswer)
            ? typeof configCorrectAnswer === "string"
              ? suppliedAnswer.length === 1 &&
                configCorrectAnswer === suppliedAnswer[0]
              : suppliedAnswer.length === configCorrectAnswer.length &&
                suppliedAnswer.every((answer) =>
                  configCorrectAnswer.includes(answer),
                )
    ...
    if (hasCorrectAnswerFeedback) {
      responsesWithDefaults.forEach((response) => {
        if (
          correctAnswers[response.id] &&
          !alertConfig[response.id]?.message.includes(
            "You've failed to answer this question correctly",
          )
        ) {
          updateAlertConfig(
            response.id,
          ...
          storeDispatch(
            saveIncorrectAnswer({
              ...
              answer: (answerValidator.values as Record<string, unknown>)[
                response.id
              ],
            }),
          );
          ...
          if (trainingAttempts === -1) {
            ...
            if (!allowFailedTraining && storageEngine) {
              storageEngine
                .rejectCurrentParticipant("Failed training")
                .then(() => {
                  setTimeout(() => {
          ...
          if (response.type === "checkbox") {
            const correct = configInUse?.correctAnswer?.find(
              (answer) => answer.id === response.id,
            )?.answer;
            ...
            const tooManySelected =
              correct.length === matches.length &&
              suppliedAnswer.length > correct.length
                ? "However, you have selected too many boxes. "
                : "";
  ...
  const nextButtonText = useMemo(
    () =>
      configInUse?.nextButtonText ??
      studyConfig.uiConfig.nextButtonText ??
      "Next",
    [configInUse, studyConfig],
  );
    ...
    <>
                ...
                <>
                  <ResponseSwitcher
                    ...
                    form={{
                      ...answerValidator.getInputProps(response.id, {
                        type:
                          response.type === "checkbox" ? "checkbox" : "input",
                      }),
                    }}
                    dontKnowCheckbox={{
                      ...answerValidator.getInputProps(
                        `${response.id}-dontKnow`,
                        { type: "checkbox" },
                      ),
                    }}
                    otherInput={{
                  ...
                  {alertConfig[response.id]?.visible && (
                    ...
                    >
                        ...
                        <>
                          ...
                          <Anchor
                            ...
                            onClick={() => {
                              ...
                              storeDispatch(
                                incrementHelpCounter({ identifier }),
                              );
                            }}
                          >
                      ...
                      <br />
                      <br />
                      {attemptsUsed >= trainingAttempts &&
                        trainingAttempts >= 0 &&
                        configCorrectAnswer &&
                        ` The correct answer was: ${configCorrectAnswer}.`}
                    </Alert>
                  )}
                </>
              )}
      ...
      {showBtnsInLocation && (
        <NextButton
          ...
          location={location}
          checkAnswer={
            showBtnsInLocation && hasCorrectAnswerFeedback ? (
              <Button
