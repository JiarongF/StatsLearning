...
function participantDataToRows(
  ...
  const percentComplete = (
    (Object.entries(participant.answers).filter(
      ([_, entry]) => entry.endTime !== -1,
    ).length /
      Object.entries(participant.answers).length) *
    100
  ).toFixed(2);
  ...
  return [
    [
      ...
      ...Object.values(participant.answers)
        .map((trialAnswer) => {
          ...
          const completeComponent = studyComponentToIndividualComponent(
            trialConfig,
            studyConfig,
          );

          const duration =
            trialAnswer.endTime === -1
              ? undefined
              : trialAnswer.endTime - trialAnswer.startTime;
          const cleanedDuration = getCleanedDuration(trialAnswer);

          const answersToLoopOver =
            Object.keys(trialAnswer.answer).length === 0
              ? { "": undefined }
              : trialAnswer.answer;
          const rows = Object.entries(answersToLoopOver)
            .map(([key, value]) => {
              ...
              Object.entries(trialAnswer.parameters).forEach(
                ([_key, _value]) => {
              ...
              const response = completeComponent.response.find(
                (resp) => resp.id === key,
              );
              if (properties.includes("status")) {
                tidyRow.status = participant.rejected
                  ? "rejected"
              ...
              if (properties.includes("rejectReason")) {
                tidyRow.rejectReason = participant.rejected
                  ? participant.rejected.reason
                  : undefined;
              }
              if (properties.includes("rejectTime")) {
                tidyRow.rejectTime = participant.rejected
                  ? new Date(participant.rejected.timestamp).toISOString()
                  : undefined;
              }
              ...
              if (properties.includes("correctAnswer")) {
                const configCorrectAnswer =
                  completeComponent.correctAnswer?.find(
                    (ans) => ans.id === key,
                  )?.answer;
                const answerCorrectAnswer = trialAnswer.correctAnswer.find(
                  (ans) => ans.id === key,
                )?.answer;
                const correctAnswer =
                  answerCorrectAnswer || configCorrectAnswer;
                tidyRow.correctAnswer =
              ...
              if (properties.includes("responseMin")) {
                tidyRow.responseMin =
                  response?.type === "numerical" ? response.min : undefined;
              }
              if (properties.includes("responseMax")) {
                tidyRow.responseMax =
                  response?.type === "numerical" ? response.max : undefined;
              }
              if (properties.includes("startTime")) {
                tidyRow.startTime = new Date(
                  trialAnswer.startTime,
                ).toISOString();
              }
...
async function getTableData(
  ...
  const allConfigHashes = [
    ...new Set(data.map((part) => part.participantConfigHash)),
  ];
  const allConfigs = await storageEngine.getAllConfigsFromHash(
    allConfigHashes,
    studyId,
  );
  ...
  const allData = await Promise.all(
    data.map(async (participant) => {
      const partDataToRows = await participantDataToRows(
        participant,
        combinedProperties,
        allConfigs[participant.participantConfigHash],
      );
...
export function DownloadTidy({
  ...
  const [selectedProperties, setSelectedProperties] = useState<
    Array<OptionalProperty>
  >([...OPTIONAL_COMMON_PROPS].filter((prop) => prop !== "meta"));
  ...
  const handlePythonExportTIDY = useCallback(() => {
    ...
    window.parent.postMessage(
      { type: "revisitWidget/PYTHON_EXPORT_TIDY", payload: tableData },
      "*",
    );
    showNotification({
      title: "Success",
      message: "Data sent to python notebook",
      color: "green",
    });
  }, [tableData]);
    ...
    >
      <MultiSelect
        ...
        onChange={(values: string[]) =>
          setSelectedProperties(values as OptionalProperty[])
        }
      ...
      <Box mih={300} style={{ width: "100%", overflow: "scroll" }}>
        ...
        ) : tableDataStatus === "error" && tableError ? (
          <Alert variant="light">{tableError.message}</Alert>
        ) : null}
        <LoadingOverlay
          visible={tableDataStatus === "pending" || tableDataStatus === "idle"}
        />
      </Box>
      ...
      <Group justify="right">
        ...
        {studyId === "__revisit-widget" && (
          <Button onClick={handlePythonExportTIDY}>
            <IconBrandPython />
          </Button>
        )}
      </Group>
