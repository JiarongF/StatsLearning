...
export type ComponentBlockWithOrderPath =
  | (Omit<ComponentBlock, "components"> & {
      orderPath: string;
      components: (ComponentBlockWithOrderPath | string)[];
      interruptions?: { components: string[] }[];
    })
  | (DynamicBlock & {
      orderPath: string;
      interruptions?: { components: string[] }[];
...
function findTaskIndexInSequence(
  sequence: Sequence,
  step: string,
  startIndex: number,
  ...
  for (let i = 0; i < sequence.components.length; i += 1) {
    ...
    if (typeof component === "string") {
      if (
        requestedPath === sequence.orderPath &&
        component === step &&
        i >= startIndex
      ) {
        break;
...
function countInterruptionsRecursively(
  ...
  participantSequence.components.forEach((component) => {
    if (
      ...
      const configSubSequence = configSequence.components.find(
        (c) => typeof c !== "string" && c.orderPath === component.orderPath,
...
function reorderComponents(
  configSequence: ComponentBlockWithOrderPath["components"],
  participantSequence: Sequence["components"],
) {
  ...
  participantSequence.forEach((sequenceComponent) => {
    ...
    const configSequenceIndex = configSequence.findIndex((c) => {
      ...
      return (
        typeof sequenceComponent !== "string" &&
        c.orderPath === sequenceComponent.orderPath
      );
    });
...
function StepItem({
  ...
  const stepIndex =
    subSequence && subSequence.components.slice(startIndex).includes(step)
      ? findTaskIndexInSequence(
          fullSequence,
          step,
          startIndex,
          subSequence.orderPath,
        )
      : -1;
  ...
  const dynamicActive =
    parentActive && parentBlock && funcIndex
      ? startIndex === decryptIndex(funcIndex)
      : false;
  const studyActive = participantView
    ? currentStep === stepIndex || dynamicActive
    : currentStep === `reviewer-${step}`;
  const active = analysisNavigation ? analysisActive : studyActive;

  const analysisNavigateTo = trialId ? `./../${step}` : `./${step}`;
  const dynamicNavigateTo =
    parentBlock.order === "dynamic"
      ? `/${studyId}/${encryptIndex(flatSequence.indexOf(parentBlock.id!))}/${encryptIndex(startIndex)}`
      : "";
  const studyNavigateTo = participantView
    ? parentBlock.order === "dynamic"
      ? dynamicNavigateTo
      : `/${studyId}/${encryptIndex(stepIndex)}`
    : `/${studyId}/reviewer-${step}`;
  const navigateTo = analysisNavigation
    ? () => navigate(analysisNavigateTo)
    : () =>
        navigate(
          `${studyNavigateTo}${participantId ? `?participantId=${participantId}` : ""}`,
        );

  const coOrComponents = step.includes(".co.")
    ? ".co."
    : step.includes(".components.")
      ? ".components."
      : false;
  const cleanedStep =
    step.includes("$") && coOrComponents && step.includes(coOrComponents)
      ? step.split(coOrComponents).at(-1)
      : step;
  ...
  const matchingAnswer =
    parentBlock.order === "dynamic"
      ? Object.entries(answers).find(
          ([key, _]) =>
            key ===
            `${parentBlock.id}_${flatSequence.indexOf(parentBlock.id!)}_${cleanedStep}_${startIndex}`,
        )
      : Object.entries(answers).find(
          ([key, _]) =>
            key === `${cleanedStep}_${startIndex}` ||
            key === `${step}_${startIndex}`,
        );
  const taskAnswer: StoredAnswer | null = matchingAnswer
    ? matchingAnswer[1]
    : null;

  const correctAnswer =
    taskAnswer &&
    taskAnswer.correctAnswer.length > 0 &&
    Object.keys(taskAnswer.answer).length > 0 &&
    taskAnswer.correctAnswer;
  const correct =
    correctAnswer &&
    taskAnswer &&
    Object.values(correctAnswer).every(
      (value) => taskAnswer.answer[value.id] === value.answer,
    );
  ...
  const [responseClamp, setResponseClamp] = useState<number | undefined>(
    INITIAL_CLAMP,
  );

  const correctAnswerJSONText =
    taskAnswer && taskAnswer.correctAnswer.length > 0
      ? JSON.stringify(taskAnswer.correctAnswer, null, 2)
      : task && task.correctAnswer
        ? JSON.stringify(task.correctAnswer, null, 2)
        : undefined;
  const [correctAnswerClamp, setCorrectAnswerClamp] = useState<
    number | undefined
  >(INITIAL_CLAMP);
          ...
          <Box mah={700} maw={500} style={{ overflow: "auto" }}>
            ...
            {("parameters" in task || taskAnswer) && (
              <Box>
                ...
                <Code block>
                  {taskAnswer &&
                    JSON.stringify(
                      Object.keys(taskAnswer).length > 0
                        ? taskAnswer.parameters
                        : "parameters" in task
                          ? task.parameters
                          : {},
                      null,
            ...
            {correctAnswerJSONText && (
              <Box>
                ...
                <Code block>
                  <Text size="xs" lineClamp={correctAnswerClamp}>
                    {correctAnswerJSONText}
                  </Text>
                  {correctAnswerJSONText.split("\n").length > INITIAL_CLAMP && (
                    <Flex justify="flex-end">
                      {(correctAnswerClamp === undefined ||
                        correctAnswerJSONText.split("\n").length >
                          correctAnswerClamp) && (
                        <Button
                          variant="light"
            ...
            <Box>
              ...
              <Code block>
                <Text size="xs" lineClamp={responseClamp}>
                  {responseJSONText}
                </Text>
                {responseJSONText.split("\n").length > INITIAL_CLAMP && (
                  <Flex justify="flex-end">
                    {(responseClamp === undefined ||
                      responseJSONText.split("\n").length > responseClamp) && (
                      <Button
                        variant="light"
                        size="xs"
                        onClick={() => {
                          setResponseClamp((prev) =>
                            prev === INITIAL_CLAMP ? undefined : INITIAL_CLAMP,
                          );
                        }}
                      >
                        {responseClamp !== undefined
...
export function StepsPanel({
  ...
  if (participantSequence && participantView) {
    const reorderedComponents = reorderComponents(
      structuredClone(configSequence.components),
      structuredClone(participantSequence.components),
    );
  ...
  if (!participantView) {
    ...
    components = [
      ...(configSequence.interruptions?.flatMap(
        (interruption) => interruption.components,
      ) || []),
      ...(components || []),
    ];
  }
  ...
  const sequenceStepsLength = useMemo(
    () =>
      participantSequence
        ? getSequenceFlatMap(participantSequence).length -
          countInterruptionsRecursively(configSequence, participantSequence)
        : 0,
    [configSequence, participantSequence],
  );
  const orderSteps = useMemo(
    () => getSequenceFlatMap(configSequence),
    [configSequence],
  );

  const [isPanelOpened, setIsPanelOpened] = useState<boolean>(
    sequenceStepsLength > 0,
  );
  ...
  // eslint-disable-next-line react-hooks/rules-of-hooks
  const flatSequence = analysisTab ? [] : useFlatSequence();
  const dynamicBlockActive =
    typeof currentStep === "number" &&
    configSequence.order === "dynamic" &&
    flatSequence[currentStep] === configSequence.id;
  const indexofDynamicBlock =
    (configSequence.id && flatSequence.indexOf(configSequence.id)) || -1;

  const studyId = useStudyId();
  const navigate = useNavigate();
  const navigateTo = () =>
    navigate(
      `/${studyId}/${encryptIndex(indexofDynamicBlock)}/${encryptIndex(0)}${participantId ? `?participantId=${participantId}` : ""}`,
    );
  ...
  const toLoopOver = [
    ...
    ...Object.entries(answers)
      .filter(([key, _]) =>
        key.startsWith(`${configSequence.id}_${indexofDynamicBlock}_`),
      )
    ...
    <NavLink
      ...
      label={
        ...
        >
          ...
          {configSequence.order === "random" ||
          configSequence.order === "latinSquare" ? (
          ...
          {participantView && (
            <Badge ml={5} variant="light">
              {configSequence.order === "dynamic"
                ? `${Object.keys(answers).filter((keys) => keys.startsWith(`${configSequence.id}_`)).length} / ?`
                : `${sequenceStepsLength}/${orderSteps.length}`}
            </Badge>
          )}
          {participantView && configSequence.interruptions && (
            <Badge ml={5} color="orange" variant="light">
              {participantSequence?.components.filter(
                (s) =>
                  typeof s === "string" &&
                  configSequence.interruptions
                    ?.flatMap((i) => i.components)
      ...
      {isPanelOpened ? (
        <Box style={{ borderLeft: "1px solid #e9ecef" }}>
          {toLoopOver.map((step, idx) => {
            if (typeof step === "string") {
              return (
                <StepItem
                  ...
                  disabled={
                    participantView &&
                    participantSequence?.components[idx] !== step
                  }
                  fullSequence={fullSequence}
                  startIndex={idx}
                  interruption={
                    (configSequence.interruptions &&
                      configSequence.interruptions.findIndex((i) =>
                        i.components.includes(step),
                      ) > -1) ||
                    false
                  }
            ...
            const newSequence = participantSequence?.components.find(
              (s) => typeof s !== "string" && s.orderPath === step.orderPath,
