...
export function StepRenderer() {
  ...
  const windowEventDebounceTime = useMemo(
    () =>
      componentConfig.windowEventDebounceTime ??
      studyConfig.uiConfig.windowEventDebounceTime ??
      100,
  ...
  useEffect(() => {
    // Focus
    const focusListener = debounce(
      (e: FocusEvent) => {
        windowEvents.current.push([
          Date.now(),
          "focus",
      ...
      { maxWait: windowEventDebounceTime },
    );
    ...
    // Keyboard
    const keydownListener = debounce(
      (e: KeyboardEvent) => {
    ...
    // Mouse/Pointer/Touch
    const mouseDownListener = debounce(
      (e: MouseEvent) => {
        windowEvents.current.push([
          Date.now(),
          "mousedown",
  ...
  const { studyNavigatorEnabled, dataCollectionEnabled } = useMemo(
    () => modes,
    [modes],
  );
  ...
  const sidebarOpen = useMemo(
    () => componentConfig.withSidebar ?? studyConfig.uiConfig.withSidebar,
    [componentConfig, studyConfig],
  );
  const sidebarWidth = useMemo(
    () =>
      componentConfig?.sidebarWidth ?? studyConfig.uiConfig.sidebarWidth ?? 300,
    [componentConfig, studyConfig],
  );
  const showTitleBar = useMemo(
    () =>
      componentConfig.showTitleBar ?? studyConfig.uiConfig.showTitleBar ?? true,
    [componentConfig, studyConfig],
  );
  ...
  return (
    <WindowEventsContext.Provider value={windowEvents}>
      ...
      >
        ...
        {showTitleBar && (
          <AppHeader
            studyNavigatorEnabled={studyNavigatorEnabled}
