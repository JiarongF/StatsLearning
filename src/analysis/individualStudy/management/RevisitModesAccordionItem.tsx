...
export function RevisitModesAccordionItem({ studyId }: { studyId: string }) {
  ...
  const [
    ...
    setAnalyticsInterfacePubliclyAccessible,
  ] = useState(false);
    ...
    asyncStatus && (
      <Stack>
        <Switch
          ...
          onChange={(event) =>
            handleSwitch("dataCollectionEnabled", event.currentTarget.checked)
          }
        />
        <Switch
          ...
          onChange={(event) =>
            handleSwitch("studyNavigatorEnabled", event.currentTarget.checked)
          }
        />
        <Switch
