...
export function DataManagementAccordionItem({
  studyId,
  refresh,
}: {
  studyId: string;
  ...
  const [modalDeleteSnapshotOpened, setModalDeleteSnapshotOpened] =
    useState<boolean>(false);
  const [modalRenameSnapshotOpened, setModalRenameSnapshotOpened] =
    useState<boolean>(false);
  const [modalDeleteLiveOpened, setModalDeleteLiveOpened] =
    useState<boolean>(false);
  ...
  const [snapshotListLoading, setSnapshotListLoading] =
    useState<boolean>(false);
  ...
  const snapshotAction = async (action: SnapshotAction) => {
    ...
    switch (action.type) {
      case "create":
        ...
        break;
      case "rename":
        response = await storageEngine.renameSnapshot(
          action.snapshot,
          action.newName,
          studyId,
        );
        break;
      case "restore":
        response = await storageEngine.restoreSnapshot(
          studyId,
          action.snapshot,
        );
        break;
      case "deleteSnapshot":
        response = await storageEngine.removeSnapshotOrLive(
          action.snapshot,
          studyId,
        );
        break;
  ...
  const handleCreateSnapshot = () =>
    snapshotAction({ type: "create", archive: false });
  const handleArchiveData = () => {
    setDeleteValue("");
    setModalArchiveOpened(false);
    snapshotAction({ type: "create", archive: true });
  };
  ...
  const handleRestoreSnapshot = (snapshot: string) =>
    snapshotAction({ type: "restore", snapshot });
  const handleDeleteSnapshot = () => {
    setDeleteValue("");
    setModalDeleteSnapshotOpened(false);
    snapshotAction({ type: "deleteSnapshot", snapshot: currentSnapshot });
  };
  const handleDeleteLive = () => {
    setDeleteValue("");
    setModalDeleteLiveOpened(false);
    snapshotAction({ type: "deleteLive" });
  };
  ...
  const openCreateSnapshotModal = () =>
    openConfirmModal({
      ...
      children: (
        <Text mb={30}>
          This will create a snapshot of the live database without impacting the
          live data.
        </Text>
      ),
  ...
  const openRestoreSnapshotModal = (snapshot: string) =>
    openConfirmModal({
      title: "Restore Snapshot",
      children: (
        <Text mb={30}>
          Restoring this snapshot will perform 2 actions. First, we will create
          a snapshot of the current live database, then we will restore the
          selected snapshot into the live database.
        </Text>
      ),
  ...
  return (
    <>
      ...
      <Flex justify="space-between" align="center">
        <Box style={{ width: "70%" }}>
          ...
          <Text>
            ...
            remove this data from the live database. The current study data can
            be restored from a snapshot at any time.
          </Text>
        </Box>

        <Tooltip label="Create a snapshot">
          <Button onClick={openCreateSnapshotModal}>Snapshot</Button>
        </Tooltip>
      </Flex>
      ...
      <Flex justify="space-between" align="center">
        <Box style={{ width: "70%" }}>
          ...
          <Text>
            ...
            database. It will then remove this data from the live database. The
            current study data can be restored from a snapshot at any time.
          </Text>
        </Box>
      ...
      <Flex justify="space-between" align="center">
        <Box style={{ width: "70%" }}>
          ...
          <Text>
            ...
            database. Since this does not create a snapshot the data will be
        ...
        <Box style={{ position: "relative" }}>
          ...
          {Object.keys(snapshots).length > 0 ? (
            <Table>
              ...
              <Table.Tbody>
                {Object.entries(snapshots).map(([key, snapshotItem]) => (
                  <Table.Tr key={key}>
                      ...
                      <Flex>
                        <Tooltip label="Rename">
                          <Button
                            ...
                            onClick={() => {
                              setModalRenameSnapshotOpened(true);
                              setCurrentSnapshot(key);
                            }}
                          >
                            <IconPencil />
                          </Button>
                        ...
                        <Tooltip label="Delete Snapshot">
                          <Button
                            ...
                            onClick={() => {
                              setModalDeleteSnapshotOpened(true);
                              setCurrentSnapshot(key);
                            }}
                          >
                            <IconTrashX />
                        ...
                        <DownloadButtons
                          visibleParticipants={() => fetchParticipants(key)}
                          studyId={studyId}
                          gap="10px"
                          fileName={snapshotItem.name}
                        />
                      </Flex>
                    </Table.Td>
                  </Table.Tr>
      ...
      >
        <Text mb="sm">
          This will create a snapshot of the live database and remove the data
      ...
      >
        ...
        <Flex mt="sm" justify="right">
          ...
          <Button
            ...
            disabled={!renameValue || renameValue.length === 0}
          >
            Rename
          </Button>
        </Flex>
      ...
      >
        <Text mb="sm">
          ...
          . Please consider using archive data to create a snapshot of the live
