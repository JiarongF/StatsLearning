...
export function AllTasksTimeline({
  ...
  const clickTask = useCallback(
    (task: string) => {
      ...
      window.open(
        `${PREFIX}${studyId}/${encryptIndex(index)}?participantId=${participantData.participantId}`,
        "_blank",
      );
    },
  ...
  const xScale = useMemo(() => {
    const allStartTimes = Object.values(participantData.answers || {})
      .filter((answer) => answer.startTime)
      .map((answer) => [answer.startTime, answer.endTime])
      .flat();
    ...
    const scale = d3
      ...
      .domain([extent[0], maxLength ? extent[0] + maxLength : extent[1]])
      .clamp(true);
  ...
  const [
    ...
    numComponentsWithCorrectAnswer,
    tasks,
  ]: [number, number, { line: JSX.Element; label: JSX.Element }[]] =
    useMemo(() => {
      let currentHeight = 0;

      const sortedEntries = Object.entries(participantData.answers || {})
        .filter((answer) => !!answer[1].startTime)
      ...
      const allElements = sortedEntries.map((entry, i) => {
        ...
        if (
          prev &&
          prev[0].length * (CHARACTER_SIZE + 1) + xScale(prev[1].startTime) >
            xScale(answer.startTime)
        ) {
          currentHeight += 1;
        } else {
          currentHeight = 0;
        ...
        if (component && component.correctAnswer) {
          component.correctAnswer.forEach((a) => {
            ...
            if (
              ...
              answer.answer[id] !== componentCorrectAnswer
            ) {
              isCorrect = false;
            }
          });
        ...
        return {
          line: (
            <SingleTaskLabelLines
              key={name}
              labelHeight={currentHeight * LABEL_GAP}
              answer={answer}
              height={height}
              xScale={xScale}
            />
              ...
              label={
                <Stack gap={0}>
                  {Object.entries(answer.answer).map((a) => {
                    ...
                    const correctAnswer = component?.correctAnswer?.find(
                      (c) => c.id === id,
                    )?.answer;
                    ...
                    return (
                      <Text
                        key={id}
                      >{`${id}: ${componentAnswer} ${correctAnswer ? `[${correctAnswer}]` : ""}`}</Text>
                    );
                  })}
                </Stack>
              }
      ...
      return [
        _numComponentsAnsweredCorrectly,
        _numComponentsWithCorrectAnswer,
        allElements,
      ];
    }, [
  ...
  const duration = useMemo(() => {
    if (
      !participantData.answers ||
      Object.entries(participantData.answers).length === 0
    ) {
      return 0;
    }

    const answersSorted = Object.values(participantData.answers)
      .filter((data) => data.startTime)
      .sort((a, b) => a.startTime - b.startTime);

    return new Date(
      answersSorted[answersSorted.length - 1].endTime -
        (answersSorted[0] ? answersSorted[0].startTime : 0),
    ).getTime();
  }, [participantData]);

  // Find entries of someone browsing away. Show them
  const browsedAway = useMemo(() => {
    const sortedEntries = Object.entries(participantData.answers || {}).sort(
    ...
    return sortedEntries.map((entry) => {
      ...
      if (answer.windowEvents) {
        for (let i = 0; i < answer.windowEvents.length; i += 1) {
          if (answer.windowEvents[i][1] === "visibility") {
            if (
              answer.windowEvents[i][2] === "hidden" &&
              currentState === "visible"
            ) {
              currentBrowsedAway = [answer.windowEvents[i][0], -1];
              currentState = "hidden";
            } else if (
              answer.windowEvents[i][2] === "visible" &&
              currentState === "hidden"
            ) {
      ...
      return browsedAwayList.map((browse, i) => (
        <Tooltip withinPortal key={i} label="Browsed away">
          <rect
            x={xScale(browse[0])}
            width={xScale(browse[1]) - xScale(browse[0])}
            y={height - 5}
            height={10}
          />
        </Tooltip>
  ...
  const partName = useMemo(
    () => participantName(participantData, studyConfig),
    [participantData, studyConfig],
  );
  ...
  const completionTime = useMemo(() => {
    ...
    return participantData.completed
      ...
      <Stack gap={15} style={{ width: "100%" }}>
        ...
        <Group justify="space-between">
          <Group justify="center">
            ...
            <Text size="md">{completionTime}</Text>

            {participantData.completed ? null : (
              <Text size="xl" c="red">
                Not completed
              </Text>
            )}
            ...
            <Group gap={10}>
              <Badge
                ...
                color="green"
                leftSection={
                  <IconCheck width={18} height={18} style={{ paddingTop: 1 }} />
                }
                pb={1}
              >
                {numComponentsAnsweredCorrectly}
              ...
              <Badge
                ...
                color="red"
                leftSection={
                  <IconX width={18} height={18} style={{ paddingTop: 1 }} />
                }
                pb={1}
              >
                {numComponentsWithCorrectAnswer -
