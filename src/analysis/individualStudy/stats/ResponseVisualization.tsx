...
export function ResponseVisualization({
  ...
  const questionData = useMemo(() => {
    ...
    const data = participantData
      .map((p) =>
        Object.entries(p.answers).filter(
          ([key]) => key.slice(0, key.lastIndexOf("_")) === trialId,
        ),
      )
      .map((p) => p.map(([, value]) => value.answer))
      .flat();
    return data;
  ...
  const vegaLiteSpec = useMemo(() => {
      ...
      const timingData = participantData
        ...
        .map((p) =>
          p
            ...
            .map(([_, value]) => (value.endTime - value.startTime) / 1000),
        )
        .flat();
    ...
    const correctAnswerSpec = {
      ...
      transform: [
        {
          calculate:
            "datum[variable] === correctAnswer ? 'correct' : 'incorrect'",
          as: "result",
        },
        {
          calculate:
            "if(datum.result === 'correct', 0, if(datum.result === 'incorrect', 1, 2))",
          as: "order",
        },
      ],
      color: {
    ...
    if (
      ...
      const spec = {
        ...
        mark: "bar",
        params:
          correctAnswer !== undefined ? correctAnswerSpec.params : undefined,
        transform: [
          {
        ...
        encoding: {
          ...
          color:
            correctAnswer !== undefined ? correctAnswerSpec.color : undefined,
          order:
            correctAnswer !== undefined ? correctAnswerSpec.order : undefined,
        },
      };
      return spec;
    }
    ...
    if (
      ...
      const spec = {
        ...
        params:
          correctAnswer !== undefined ? correctAnswerSpec.params : undefined,
        transform:
          correctAnswer !== undefined ? correctAnswerSpec.transform : undefined,
        encoding: {
    ...
    <Paper p="lg" withBorder ref={ref}>
      <Flex direction="row" justify="space-between" onClick={toggle}>
        <Flex align="center">
          ...
          {(response.type === "shortText" || response.type === "longText") && (
              ...
              <>
                ...
                {correctAnswer && (
                  <>
                    ...
                    <Code block>{JSON.stringify(correctAnswer, null, 2)}</Code>
                  </>
                )}
              </>
            )}
