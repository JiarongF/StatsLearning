...
type Listeners = {
  [key: string]: (
    key: string,
    value: { responseId: string; response: string | number },
  ) => void;
};
...
export function VegaController({
  currentConfig,
  provState,
}: {
  currentConfig: VegaComponent;
  provState?: VegaProvState;
}) {
  ...
  const { updateResponseBlockValidation, setReactiveAnswers } =
    useStoreActions();
  ...
  const handleSignalEvt = useCallback(
  ...
  const handleRevisitAnswer = useCallback(
    (key: string, value: Parameters<ValueOf<Listeners>>[1]) => {
